...
3. **Collect ~12 to 20 of their ORIGINAL posts.** Ignore reposts and pure link-shares with no commentary. The activity list lazy-loads after a few seconds, and `get_page_text` will NOT capture it (it only extracts article-style content, so it returns just the "All activity" heading). Use `javascript_tool` innerText reads instead. A selector that works on the activity page: `document.querySelectorAll('.update-components-text, .feed-shared-inline-show-more-text')`, mapped to `innerText`, filtered to entries longer than ~60 chars. One read typically returns ~10 of their posts; scroll or reload-sample to gather more. If the tab freezes in the background, use the frozen-tab workarounds (synchronous `javascript_tool` reads, reload-sampling) described in `references/apple-notes-sync.md`. Accumulate the post text in `localStorage` and read it out in chunks if results truncate.
